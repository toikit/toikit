...
function declareModel(names) {
    function binding(name) {
        if (typeof name == 'object')
            name = name.name;
        if ((0, inject_1.getDeclaration)('model_' + name))
            return;
        let attributes = (0, data_1.getData)('model_attributes_' + name);
        let options = (0, data_1.getData)('model_options_' + name);
        let mounted = (0, data_1.getData)('model_mounted_' + name);
        let conn = (0, data_1.getData)('model_connection_' + name);
...
function modelData(models) {
    function binding(data) {
        ...
        (0, data_1.setData)('model_attributes_' + name, data.attributes);
        (0, data_1.setData)('model_connection_' + name, conn);
        (0, data_1.setData)('model_options_' + name, data.options || {});
        (0, data_1.setData)('model_mounted_' + name, data.mounted ? [data.mounted] : []);
    }
